...
ggml_tensor * build_attention_block(
    ...
    const int H        = static_cast<int>(x->ne[0]);
    const int S        = static_cast<int>(x->ne[1]);
    const int B        = static_cast<int>(x->ne[2]);
    const int head_dim = H / n_head;
    const float scale  = 1.0f / std::sqrt(static_cast<float>(head_dim));

    // Linear projections produce [H, S, B] each.
    ggml_tensor * q = ggml_add(ctx, ggml_mul_mat(ctx, w.q_w, x), w.q_b);
    ggml_tensor * k = ggml_add(ctx, ggml_mul_mat(ctx, w.k_w, x), w.k_b);
    ggml_tensor * v = ggml_add(ctx, ggml_mul_mat(ctx, w.v_w, x), w.v_b);
    ...
    ggml_tensor * y      = ggml_add(ctx, x, attn);
