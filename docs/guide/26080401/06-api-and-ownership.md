...
| `max_seq_len` | 512 | 토큰화 상한과 활성값 버퍼 예약 길이 |
| `use_streaming` | 0 | 1은 아직 지원하지 않아 컨텍스트 생성 실패 |
| `pooling` | Mean | Mean 또는 CLS |
...
긴 문맥 모델의 전체 상한을 무조건 예약하지 않는 것도 중요하다. 어텐션 점수 텐서는 토큰 수의 제곱에 비례한다. 문맥 8,192인 모델을 기본값으로 전부 예약하면 사용하지 않을 수 GB의 활성값 공간이 필요할 수 있다. 그래서 공개 API 기본 상한은 512다.
