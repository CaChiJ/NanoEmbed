...
- `eurobert`이면 종류를 알아본 뒤 아직 미구현이라는 오류를 반환한다.
...
- `bert` 토크나이저는 현재 지원하는 WordPiece다.
...
### M3.6 — 모델 교체 구조와 eurobert: 일부 완료

구조 분리는 완료했다.

- `ModelArch` 인터페이스와 `general.architecture` 레지스트리
- `Tokenizer` 인터페이스와 `tokenizer.ggml.model` 레지스트리
- BERT 구현을 `BertModelArch` 뒤로 이동
- Mean, CLS, Last 내부 풀링 종류
- 벤치 시나리오에서 Hugging Face 캐시 파일을 지정하는 모델 경로

eurobert 실행은 아직 구현하지 않았다.

- RoPE 위치 인코딩
- RMSNorm
- SwiGLU FFN
- 편향값이 없는 선형 변환
- GPT-2 계열 byte-level BPE
- jina-v5-nano 토큰 ID와 최종 임베딩 기준 데이터

`jina-embeddings-v5-text-nano-retrieval`은 BERT와 텐서 구성이 거의 겹치지 않는다. 따라서 BERT 코드에 조건문 몇 개를 추가하지 않고 별도 모델 구조 구현으로 추가한다.

Q3_K_M 가중치가 기존 `ggml_mul_mat`과 `ggml_get_rows`에서 별도 변환 없이 동작할지는 아직 가정이다. F16으로 먼저 결과 정합성을 확인한 뒤 양자화 모델을 검증한다.
...
- `use_streaming=1` 지원
- bge-small과 더 큰 eurobert 모델에서 스트리밍 전후 RSS 비교
...
- eurobert의 Q3_K_M 텐서가 필요한 모든 ggml 연산에서 지원되는지 확인해야 한다.
